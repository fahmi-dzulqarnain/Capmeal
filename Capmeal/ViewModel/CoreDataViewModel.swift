...
class CoreDataViewModel: ObservableObject {
    ...
    @Published var profileData: ProfileEntity? = nil
    ...
    func fetchJournal(){
        do {
            journalData = try context.fetch(NSFetchRequest<JournalEntity>(entityName: "JournalEntity"))
        } catch let error {
            print("Error nih pas ngambil data jurnalnya, nih detailnya \(error)")
        }
    }
    
    func fetchJournalBaseOnDay(tanggalWaktu: Date){
        do {
            ...
            journalData = try context.fetch(request)
        } catch let error {
            print("Error nih pas ngambil data jurnalnya, nih detailnya \(error)")
        }
    }
    
    func fetchJournalBaseOnDayAndType(tanggalWaktu: Date, tipe: String){
        do {
            let request = NSFetchRequest<JournalEntity>(entityName: "JournalEntity")
    ...
    func fetchProfile(){
        do {
            profileData = try context.fetch(NSFetchRequest<ProfileEntity>(entityName: "ProfileEntity"))[0]
        } catch let error {
            print("Error nih pas ngambil data jurnalnya, nih detailnya \(error)")
        }
    }
